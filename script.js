...
req.onload = function (e) {
  ...
  for (let i = 0; i < json_sheet.length; i++) {
    ...
    var date = new Date(s['Update Date']);
    ...
    card.dataset.faq = s["FAQ"]
    card.dataset.answer = s["Answer"]
    card.dataset.discussionType = s['Discussion Type']
    card.dataset.family = s['Family']
    card.dataset.application = s['Application']
    card.dataset.area = s['Area']
    card.dataset.subArea = s['Sub-Area']
    card.dataset.productVersion = s['Product Version']
    card.dataset.company = s['Company']
    card.dataset.serialDate = s['Update Date']
...
function create_filters(sheet) {
  ...
  options.forEach(function (fetched_option) { //option = filter label
    ...
    if (fetched_option === "Discussion Type" || fetched_option === "Family" || fetched_option === "Application" || fetched_option === "Area" || fetched_option === "Sub-Area" || fetched_option === "Product Version" || fetched_option === "Company") {
...
function dragElement(elmnt) {
  ...
  function elementDrag(e) {
    ...
    if (elmnt.id === "max") {
      ...
      if (current_max <= 250 && current_max > current_min + 20) {
        elmnt.style.left = (elmnt.offsetLeft - pos1) + "px";
        max_date = 42400 + current_max*4;
...
function filter_date() {
  ...
  for (let i = 0; i < cards.length; i++) {
    ...
    if (date < min_date || date > max_date || cards[i].dataset.serialDate === "undefined") {
...
function search(text) {
  ...
  for (let i = 0; i < json_sheet.length; i++) {
    s = json_sheet[i];
    var tags = s['Discussion Type'] + s['Family'] + s['Application'] + s['Area'] + s['Sub-Area'] + s['Product Version'] + s['Company'] + s['FAQ'] + s["Update Date"] + cards[i].dataset.time;
...
function pop(element) {
  ...
  document.getElementById("popup_title").innerHTML = element.dataset['faq']
  document.getElementById("popup_description").innerHTML = element.dataset['answer']
}
